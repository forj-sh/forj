/**
 * Sentry Instrumentation for Forj Workers
 * 
 * IMPORTANT: This file must be imported FIRST in start-workers.ts
 * Initializes Sentry with privacy controls for background job monitoring
 */

import * as Sentry from '@sentry/node';

const SENTRY_DSN = process.env.SENTRY_DSN_WORKERS;
...
if (SENTRY_DSN) {
  Sentry.init({
    ...
    release: process.env.GIT_COMMIT_SHA || 'forj-workers@unknown',
  ...
  console.log('[Sentry] Initialized for ' + SENTRY_ENVIRONMENT + ' environment (workers)');
} else {
  console.log('[Sentry] Disabled - no DSN provided (workers)');
}
...
/**
 * Capture job failure with context
 */
export function captureJobError(error: Error, jobContext: {
  jobId: string;
  queueName: string;
  jobType: string;
  attemptsMade: number;
  ...
  Sentry.withScope((scope) => {
    scope.setContext('job', {
      ...
      queueName: jobContext.queueName,
      jobType: jobContext.jobType,
      attemptsMade: jobContext.attemptsMade,
    });
    
    // Scrub sensitive data from job data before sending
    if (jobContext.data) {
      const scrubbedData = JSON.parse(scrubSensitiveData(JSON.stringify(jobContext.data)));
      scope.setContext('jobData', scrubbedData);
    }
